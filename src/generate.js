...
async function isValidDate(dags) {
  const dagsetning = /^2024/;
  return dagsetning.test(dags);
}
...
async function main() {
  ...
  for await (const file of files) {
    ...
    const leikir = []
    leikir.push(parsed.games)

    /** console.log('leikir', parsed.games)*/

    const dags = []
    if (isValidDate(parsed.date)) {
      dags.push(parsed.date);
      console.log("Valid date", parsed.date);
    }

    /**console.log('dags', parsed.date);*/
